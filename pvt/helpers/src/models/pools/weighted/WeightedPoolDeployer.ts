...
export default {
  ...
  async _deployStandalone(params: WeightedPoolDeployment, vault: Vault): Promise<Contract> {
    ...
    switch (poolType) {
      case WeightedPoolType.WEIGHTED_ORACLE_POOL: {
        result = deploy('v2-pool-weighted/MockWeightedOraclePool', {
          args: [
            {
              vault: vault.address,
              name: NAME,
  ...
  async _deployFromFactory(params: WeightedPoolDeployment, vault: Vault): Promise<Contract> {
    ...
    switch (poolType) {
      case WeightedPoolType.WEIGHTED_ORACLE_POOL: {
        const factory = await deploy('v2-pool-weighted/WeightedOraclePoolFactory', {
          args: [vault.address],
          from,
        ...
        result = deployedAt('v2-pool-weighted/WeightedOraclePool', event.args.pool);
        break;
      }
      case WeightedPoolType.LIQUIDITY_BOOTSTRAPPING_POOL: {
