...
async function main() {
  ...
  console.log(`\n# Investment Pool with 80 tokens`);

  await multihop((index: number) => getWeightedPool(vault, tokens, 80, index), false);
  await multihop((index: number) => getWeightedPool(vault, tokens, 80, index), true);
