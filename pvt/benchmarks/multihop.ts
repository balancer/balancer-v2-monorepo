...
async function main() {
  ...
  console.log(`\n# Investment Pool with 81 tokens`);

  await multihop((index: number) => getWeightedPool(vault, tokens, 81, index), false);
  await multihop((index: number) => getWeightedPool(vault, tokens, 81, index), true);
