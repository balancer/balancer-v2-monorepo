...
async function main() {
  ...
  console.log(`\n# Managed Pool with 40 tokens`);

  await multihop((index: number) => getWeightedPool(vault, tokens, 40, index), false);
  await multihop((index: number) => getWeightedPool(vault, tokens, 40, index), true);
