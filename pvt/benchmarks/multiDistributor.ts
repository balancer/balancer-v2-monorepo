...
async function createDistributions(
  ...
  for (let i = 0; i < numberOfDistributions; i++) {
    ...
    await distributor.fundDistribution(distributionChannelId, DISTRIBUTION_AMOUNT, { from: distributionOwner });

    distributionIds.push(distributionChannelId);
  }
...
async function claimDistributions(numberOfDistributions: number, useInternalBalance: boolean) {
  ...
  const distributionChannelIds = await createDistributions(
    distributor,
    stakingToken,
    distributionToken,
    numberOfDistributions
  ...
  // Subscribe
  await distributor.subscribe(distributionChannelIds, { from: trader });
