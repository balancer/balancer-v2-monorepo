...
describe('CWPTradingStrategy', function () {
  ...
  describe('TS Creation', () => {
    ...
    it('Fails creating below MIN WEIGHT', async () => {
      await expect(
        CWPTradingStrategyFactory.deploy(generateAddressArray(2), [0, 8], 2, (0.05e18).toString())
      ).to.be.revertedWith('ERR_MIN_WEIGHT');
    });
    it('Fails creating below MIN TOKENS', async () => {
      await expect(
        CWPTradingStrategyFactory.deploy(generateAddressArray(1), [8], 1, (0.05e18).toString())
