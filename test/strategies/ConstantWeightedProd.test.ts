...
describe('ConstantWeightedProdStrategy', function () {
  ...
  describe('TS Creation', () => {
    it('Creates correctly TS', async () => {
      strategy = await ConstantWeightedProdStrategyFactory.deploy(packWeights([2, 8]), 2, (0.05e18).toString());
      ...
      strategy = await ConstantWeightedProdStrategyFactory.deploy(
        packWeights([2.15, 24.3, 12.11, 2, 6]),
        5,
        (0.05e18).toString()
      );
    ...
    it('Fails creating below MIN WEIGHT', async () => {
      await expect(
        ConstantWeightedProdStrategyFactory.deploy(packWeights([0, 8]), 2, (0.05e18).toString())
      ).to.be.revertedWith('ERR_MIN_WEIGHT');
    });
    it('Fails creating below MIN TOKENS', async () => {
      await expect(
        ConstantWeightedProdStrategyFactory.deploy(packWeights([8]), 1, (0.05e18).toString())
  ...
  describe('Pair balances validation', () => {
    it('Validates correctly two tokens', async () => {
      //weights: [8, 2]
      strategy = await ConstantWeightedProdStrategyFactory.deploy(packWeights([8, 2]), 2, (0.05e18).toString()); //fee: 0.05%
    ...
    it('Validates correctly three tokens', async () => {
      //weights: [4, 4, 2]
      strategy = await ConstantWeightedProdStrategyFactory.deploy(packWeights([4, 4, 2]), 3, (0.05e18).toString()); //fee: 0.05%
      await strategy.deployed();
      const result = await strategy.validatePair(
        poolID,
        0,
