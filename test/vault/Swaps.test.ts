...
describe('Vault - swaps', () => {
  ...
  describe('swap given in', () => {
    ...
    it('only transfers tokens for the net vault balance change', async () => {
      ...
      await expectBalanceChange(
        () =>
          vault.connect(other).batchSwapGivenIn(ZERO_ADDRESS, '0x', swaps, tokenAddresses, {
            ...funds,
  ...
  describe('swap given out', () => {
    it('single pair single pool swap', async () => {
      ...
      await expectBalanceChange(
        ...
        tokens,
        { account: trader, changes: { DAI: 1e18, MKR: -0.5e18 } }
      );
    });
    ...
    it('only transfers tokens for the net vault balance change', async () => {
      ...
      await expectBalanceChange(
        () =>
          vault.connect(other).batchSwapGivenOut(ZERO_ADDRESS, '0x', swaps, tokenAddresses, {
            ...funds,
            sender: other.address,
            recipient: other.address,
          }),
  ...
  describe('funds', () => {
    ...
    it('can send funds to arbitrary recipient', async () => {
      await expectBalanceChange(
        ...
        tokens,
        { account: other, changes: { DAI: 2e18 } } // The MKR is deducted from trader
      );
    });
    ...
    it('can withdraw from user balance before pulling tokens', async () => {
      ...
      await expectBalanceChange(
        ...
        tokens,
        { account: trader, changes: { DAI: 2e18, MKR: -0.7e18 } } // The 0.3e18 remaining came from User Balance
      );
    ...
    it('can deposit into user balance', async () => {
      await expectBalanceChange(
        ...
        tokens,
        { account: trader, changes: { MKR: -1e18 } }
      );
