...
import { fromRpcSig } from '@ethereumjs/ethereumjs-util';
...
describe('ERC20Permit', function () {
  ...
  beforeEach('set up asset manager', async () => {
    token = await deploy('MockBalancerPoolToken', { args: [name, symbol, initialHolder, initialSupply] });
  ...
  it('initial nonce is 0', async function () {
    expect(await this.token.nonces(initialHolder)).to.equal(0);
  });
