...
/**
 * @title "v1 style" Pools
 * @author Balancer Labs
 * @notice This contract relies on tons of immutable state variables to perform efficient lookup,
 *         without resorting to storage reads.
 */
contract ConstantProductPool is IBPTPool, IPairTradingStrategy, BToken, ConstantProductMath, ReentrancyGuard {
    ...
    /**
     * @notice Get the weights of a set of pool tokens
     * @param tokens - array of token addresses
     * @return corresponding token weights
     */
    ...
    /**
     * @notice Getter for the pool swap fee
     * @return swap fee
     */
     ...
     * @param request - data required for a price quote (especially the tokens, and amountIn)
     * @param currentBalanceTokenIn - pool balance of the token coming in
     * @param currentBalanceTokenOut - pool balance of the token going out
    ...
    // Get the weight of a pool token
