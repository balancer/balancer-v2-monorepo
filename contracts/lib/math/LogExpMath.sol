...
library LogExpMath {
    ...
    function n_exp(int256 x) internal pure returns (int256) {
        require(x >= EXPONENT_LB && x <= EXPONENT_UB, "N_EXP_ARG_OUT_OF_BOUNDS");
    ...
    function n_log(int256 a) internal pure returns (int256) {
        require(a > 0, "ERR_NON_NEGATIVE_LOG");
    ...
    function pow(uint256 x, uint256 y) internal pure returns (uint256) {
        ...
        require(x < 2**255, "x >= 2**255"); // uint256 can be casted to a positive int256
        require(y < MILD_EXPONENT_BOUND, "y >= 2**254 / 10**20");
