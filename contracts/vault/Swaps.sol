    ...
    function queryBatchSwap(
        ...
        if (msg.sender != address(this)) {
            ...
            assembly {
                ...
                switch success
                    case 0 {
                        ...
                        // The returndata now contains the raw memory representation of an array: length + data. We
                        // need to return an ABI-encoded representation of this array. An ABI-encoded array contains
                        // an additional field when compared to its raw memory representation: an offset to the
                        // location of the length. The offset itself is 32 bytes long, so the smallest value we can
                        // use is 32 for the data to be located immediately after it.
                        mstore(0, 32)

                        // We now copy the raw memory array from returndata into memory. Since the offset takes up 32
                        // bytes, we start copying at address 0x20. We also get rid of the error signature.
            ...
            assembly {
                ...
                // We store it in the previous slot to the `deltas` array. We can make sure there
