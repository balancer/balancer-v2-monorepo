...
abstract contract StablePoolRates is StablePoolStorage {
    ...
    function _updateOldRates() internal {
        ...
        if (totalTokens > 3 && _hasRateProvider(3)) _updateOldRate(3);
        if (totalTokens > 4 && _hasRateProvider(4)) _updateOldRate(4);
        if (totalTokens > 5 && _hasRateProvider(5)) _updateOldRate(5);
    }
