...
contract DistributionScheduler is IDistributionScheduler {
    ...
    function getScheduleId(bytes32 distributionChannelId, uint256 startTime) public pure override returns (bytes32) {
        return keccak256(abi.encodePacked(distributionChannelId, startTime));
    }
    ...
    function scheduleDistribution(
        ...
        scheduleId = getScheduleId(distributionChannelId, startTime);
        require(
        ...
        IMultiDistributor.DistributionChannel memory distributionChannel = _multiDistributor.getDistributionChannel(
        ...
        emit DistributionScheduled(distributionChannelId, scheduleId, startTime, amount);
    }
    ...
    function startDistributions(bytes32[] calldata scheduleIds) external override {
        for (uint256 i; i < scheduleIds.length; i++) {
            ...
            IERC20 distributionToken = _multiDistributor
                .getDistributionChannel(scheduledDistribution.distributionChannelId)
                .distributionToken;

            distributionToken.approve(address(_multiDistributor), scheduledDistribution.amount);
            _multiDistributor.fundDistribution(
                scheduledDistribution.distributionChannelId,
                scheduledDistribution.amount
            );

            emit DistributionStarted(scheduledDistribution.distributionChannelId, scheduleId);
        }
    }
    ...
    function cancelDistribution(bytes32 scheduleId) external override {
        ...
        emit DistributionCancelled(scheduledDistribution.distributionChannelId, scheduleId);
    }
}
