...
interface IDistributionScheduler {
    ...
    event DistributionStarted(bytes32 indexed distributionChannel, bytes32 indexed scheduleId);
    event DistributionCancelled(bytes32 indexed distributionChannel, bytes32 indexed scheduleId);

    // Getters

    function getScheduledDistributionInfo(bytes32 scheduleId) external view returns (ScheduledDistribution memory);

    function getScheduleId(bytes32 distributionChannelId, uint256 startTime) external pure returns (bytes32);
