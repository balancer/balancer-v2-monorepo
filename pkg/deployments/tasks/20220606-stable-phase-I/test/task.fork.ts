...
import { Account, StablePoolEncoder, SwapKind } from '@balancer-labs/balancer-js';
...
describe('StablePool', function () {
  ...
  const task = new Task('20220606-stable-phase-I', TaskMode.TEST, getForkedNetwork(hre));
  ...
  it.skip('rejects invalid update', async () => {
    await expect(pool.connect(owner).startAmplificationParameterUpdate(bn(1000), 2**33)).to.be.revertedWith('BAL#436');
  });

  // This will fail until curvier-math merged
  it.skip('converges when unbalanced', async () => {
  ...
  context('permissioned functions', () => {
    ...
    before('impersonate the multi-sign and grant permissions', async () => {
      ...
      await authorizer
        .connect(govMultisig)
        .grantRole(await actionId(factory, 'disable'), govMultisig.address);
      /*await authorizer
        .connect(govMultisig)
        .grantRole(await actionId(pool, 'enterRecoveryMode'), govMultisig.address);
      await authorizer
        .connect(govMultisig)
        .grantRole(await actionId(pool, 'exitRecoveryMode'), govMultisig.address);*/
    });

    // Skip until recovery-mode merged
    it.skip('cannot perform a recovery exit until enabled', async () => {
      const bptBalance = await pool.balanceOf(whale.address);

      // Use BasePoolEncoder when merged
      const userData = defaultAbiCoder.encode(['uint256', 'uint256'], [255, bptBalance]);

      await expect(vault.connect(whale).exitPool(poolId, whale.address, owner.address, {
        assets: tokens,
    ...
    it.skip('can perform a recovery mode exit when enabled', async () => {
      ...
      const whaleUSDCBalanceBeforeExit = await usdc.balanceOf(whale.address);

      // Use BasePoolEncoder when merged
      const userData = defaultAbiCoder.encode(['uint256', 'uint256'], [255, bptBalance]);

      await vault.connect(whale).exitPool(poolId, whale.address, owner.address, {
        assets: tokens,
        minAmountsOut: Array(tokens.length).fill(0),
        fromInternalBalance: false,
        userData,
      ...
      const remainingBalance = await pool.balanceOf(whale.address);
      expect(remainingBalance).to.equal(0);

      const vaultUSDCBalanceAfterExit = await usdc.balanceOf(vault.address);
      const whaleUSDCBalanceAfterExit = await usdc.balanceOf(whale.address);
      
      expect(vaultUSDCBalanceAfterExit).to.lt(vaultUSDCBalanceBeforeExit);
      expect(whaleUSDCBalanceAfterExit).to.lt(whaleUSDCBalanceBeforeExit);
    });
    ...
    it('can disable the factory', async () => {
      ...
      await expect(factory.create('SP3', 'SPT3', tokens, amplificationParameter, swapFeePercentage, owner.address)).to.be.revertedWith('BAL#211')
    });
  })
});
