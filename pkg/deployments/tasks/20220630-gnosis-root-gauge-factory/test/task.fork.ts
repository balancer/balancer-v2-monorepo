...
// We currently do not have a GaugeAdder which supports deploying gauges with a type of "Gnosis".
...
describe('GnosisRootGaugeFactory', function () {
  ...
  const task = new Task('20220630-gnosis-root-gauge-factory', TaskMode.TEST, getForkedNetwork(hre));
  ...
  it('create gauge', async () => {
    ...
    const event = expectEvent.inReceipt(await tx.wait(), 'GnosisRootGaugeCreated');
