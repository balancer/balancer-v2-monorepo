...
describeForkTest('MidasWrapping', 'bsc', 26028991, function () {
  let task: Task;
  let relayer: Contract, library: Contract;
  let vault: Contract, authorizer: Contract;

  // Some other options to consider - paused now
  // old: 23696722
  ...
  // wbnb in this pool
  // https://development.midascapital.xyz/56/pool/1
  const WBNB = '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c';
  const WBNB_HOLDER = '0xd7D069493685A581d27824Fc46EdA46B7EfC0063';
  ...
  before('run task', async () => {
    task = new Task('20230214-midas-batch-relayer', TaskMode.TEST, getForkedNetwork(hre));
  ...
  it('should wrap successfully', async () => {
    ...
    const depositIntoMidas = library.interface.encodeFunctionData('wrapMidas', [
      cWBNB,
      sender.address,
      relayer.address,
      amountToWrap,
    ...
    expect(balanceOfcwbnbAfter).to.be.almostEqual(expectedBalanceOfcwbnbAfter, 1);
  });
  ...
  it('should unwrap successfully', async () => {
    ...
    const withdrawFromMidas = library.interface.encodeFunctionData('unwrapMidas', [
      cWBNB,
      relayer.address,
      sender.address,
      chainedReference,
