...
describeForkTest('SiloWrapping', 'mainnet', 16622559, function () {
    ...
    let usdcToken: Contract, wrappedToken: Contract, shareToken: Contract, silo: Contract;
    ...
    it('should wrap successfully', async () => {
        ...
        expect(balanceOfWrappedAfter).to.be.almostEqual(expectedBalanceOfWrappedAfter, .01);
    });

    it('should unwrap successfully', async () => {
        // Hardcode unwrapping amount due to no exchange rate function exposed
        const estimatedRate = await siloExchangeRate(silo, USDC, wrappedToken);
        console.log("estimated Rate: ", estimatedRate);
        const wrappedRate = Math.floor(1e6 / estimatedRate);
        console.log("wrapped Rate: ", wrappedRate);
        ...
        expect(balanceOfWrappedAfter).to.be.almostEqual(amountToWrap - amountToWithdraw, .01);
