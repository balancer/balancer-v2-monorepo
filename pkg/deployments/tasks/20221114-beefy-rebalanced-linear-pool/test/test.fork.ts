...
describeForkTest('BeefyLinearPoolFactory', 'optimism', 38313066, function () {
  ...
  before('run task', async () => {
    task = new Task('20221114-beefy-rebalanced-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
    await task.run({ force: true });
    factory = await task.deployedInstance('BeefyLinearPoolFactory');
  });
  ...
  describe('create, join, and rebalance', () => {
    it('deploy a linear pool', async () => {
      ...
      rebalancer = await task.instanceAt('BeefyLinearPoolRebalancer', assetManager);
