...
describeForkTest('VotingEscrowRemapper', 'mainnet', 16971700, function () {
  ...
  before('run task', async () => {
    task = new Task('20230403-vebal-remapper', TaskMode.TEST, getForkedNetwork(hre));
  ...
  before('grant register and rename permissions to admin', async () => {
    ...
    await authorizer.connect(govMultisig).grantRole(await actionId(veRemapper, 'setOmniVotingEscrow'), admin.address);
    await authorizer
      .connect(govMultisig)
  ...
  before('set omni voting escrow', async () => {
    await veRemapper.connect(admin).setOmniVotingEscrow(omniVotingEscrow.address);
  });
