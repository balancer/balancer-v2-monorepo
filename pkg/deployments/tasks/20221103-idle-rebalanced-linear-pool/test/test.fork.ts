...
describeForkTest('IdleLinearPoolFactory', 'mainnet', 15225000, function () {
  ...
  const idleUSDC = '0x5274891bEC421B39D23760c04A6755eCB444797C';
  ...
  before('run task', async () => {
    task = new Task('20221103-idle-rebalanced-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
    await task.run({ force: true });
    factory = await task.deployedInstance('IdleLinearPoolFactory');
  });
  ...
  describe('create, join, and rebalance', () => {
    it('deploy a linear pool', async () => {
      const tx = await factory.create('', '', USDC, idleUSDC, INITIAL_UPPER_TARGET, SWAP_FEE_PERCENTAGE, owner.address);
      const event = expectEvent.inReceipt(await tx.wait(), 'PoolCreated');

      pool = await task.instanceAt('IdleLinearPool', event.args.pool);
      ...
      const { assetManager } = await vault.getPoolTokenInfo(poolId, USDC); // We could query for either USDC or idleUSDC
      rebalancer = await task.instanceAt('IdleLinearPoolRebalancer', assetManager);
