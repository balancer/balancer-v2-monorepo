...
async function runForkTests(args: any, hre: HardhatRuntimeEnvironment, run: RunSuperFunction<any>): Promise<void> {
  ...
  // Run each test file in their own brand-new fork
  ...
  // Since we're now effectively running the test task multiple times, and each run sets the process' exit code, we need
  // to overwrite that ourselves (by erroring if there were any errors).
  process.exitCode = sum(results);
}
