...
contract veBALDeploymentCoordinator is BaseCoordinator {
    ...
    ) BaseCoordinator(authorizerAdaptor, 2) {
    ...
    function _performNextStage(uint256 currentStage) internal override {
        if (currentStage == 0) {
            _performFirstStage();
    ...
    function _checkReady(uint256 currentStage) internal view override {
        if (currentStage == 0) {
    ...
    function _performFirstStage() internal {
    ...
    function _performSecondStage() internal {
        ICurrentAuthorizer authorizer = getAuthorizer();
    ...
    function _performThirdStage() internal {
