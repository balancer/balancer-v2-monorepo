...
import "./IdleLinearPoolRebalancer.sol";

contract IdleLinearPoolFactory is ILastCreatedPoolFactory, BasePoolFactory, ReentrancyGuard, FactoryWidePauseWindow {
    ...
    /**
     * @dev Deploys a new `IdleLinearPool`.
     */
    function create(
        string memory name,
        ...
        // We are going to deploy both an IdleLinearPool and an IdleLinearPoolRebalancer set as its Asset Manager, but
        ...
        bytes memory rebalancerCreationCode = abi.encodePacked(
            type(IdleLinearPoolRebalancer).creationCode,
        ...
        IdleLinearPool.ConstructorArgs memory args = IdleLinearPool.ConstructorArgs({
            vault: getVault(),
            name: name,
            symbol: symbol,
            mainToken: mainToken,
        ...
        IdleLinearPool pool = IdleLinearPool(_create(abi.encode(args)));
