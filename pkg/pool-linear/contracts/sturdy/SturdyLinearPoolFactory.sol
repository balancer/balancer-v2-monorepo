...
import "./SturdyLinearPool.sol";
import "./SturdyLinearPoolRebalancer.sol";

contract SturdyLinearPoolFactory is ILastCreatedPoolFactory, BasePoolFactory, ReentrancyGuard, FactoryWidePauseWindow {
    ...
    ) BasePoolFactory(vault, protocolFeeProvider, type(SturdyLinearPool).creationCode) {
        _queries = queries;
    }
    ...
    /**
     * @dev Deploys a new `SturdyLinearPool`.
     */
    function create(
        string memory name,
        ...
        address owner
    ) external nonReentrant returns (SturdyLinearPool) {
        // We are going to deploy both an SturdyLinearPool and an SturdyLinearPoolRebalancer set as its Asset Manager,
        ...
        bytes memory rebalancerCreationCode = abi.encodePacked(
            type(SturdyLinearPoolRebalancer).creationCode,
        ...
        SturdyLinearPool.ConstructorArgs memory args = SturdyLinearPool.ConstructorArgs({
            vault: getVault(),
            name: name,
            symbol: symbol,
            mainToken: mainToken,
        ...
        SturdyLinearPool pool = SturdyLinearPool(_create(abi.encode(args)));
