...
describe('CompoundLinearPoolFactory', function () {
  ...
  sharedBeforeEach('deploy factory & tokens', async () => {
    ...
    factory = await deploy('CompoundLinearPoolFactory', {
    ...
    const wrappedTokenInstance = await deploy('MockCToken', { args: ['cDAI', 'cDAI', 8, mainToken.address] });
  ...
  describe('constructor arguments', () => {
    ...
    it('sets a rebalancer as the asset manager', async () => {
      ...
      const rebalancer = await deployedAt('CompoundLinearPoolRebalancer', assetManager);
