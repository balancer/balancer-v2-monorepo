...
contract ManagedPool is ManagedPoolSettings {
    ...
    function _onJoinSwap(
        ...
        if (request.kind == IVault.SwapKind.GIVEN_IN) {
            ...
            amountCalculated = _weightedMath.calcBptOutGivenExactTokenIn(
                balanceTokenIn,
                tokenInWeight,
                request.amount,
                actualSupply,
            ...
            // scaling.
            amountCalculated = _weightedMath.calcTokenInGivenExactBptOut(
                balanceTokenIn,
                tokenInWeight,
                request.amount,
                actualSupply,
    ...
    function _onExitSwap(
        ...
        if (request.kind == IVault.SwapKind.GIVEN_IN) {
            ...
            // scaling.
            amountCalculated = _weightedMath.calcTokenOutGivenExactBptIn(
                balanceTokenOut,
                tokenOutWeight,
                request.amount,
                actualSupply,
            ...
            amountCalculated = _weightedMath.calcBptInGivenExactTokenOut(
                balanceTokenOut,
                tokenOutWeight,
                request.amount,
                actualSupply,
    ...
    function _onTokenSwap(
        ...
        if (request.kind == IVault.SwapKind.GIVEN_IN) {
            ...
            amountCalculated = _weightedMath.calcOutGivenIn(
                balanceTokenIn,
            ...
            uint256 amountInMinusFees = _weightedMath.calcInGivenOut(
                balanceTokenIn,
                tokenData.tokenInWeight,
                balanceTokenOut,
                tokenData.tokenOutWeight,
    ...
    function _doJoin(
        ...
        if (kind == WeightedPoolUserData.JoinKind.EXACT_TOKENS_IN_FOR_BPT_OUT) {
            return
                _weightedMath.joinExactTokensInForBPTOut(
                    balances,
                    normalizedWeights,
                    scalingFactors,
                    totalSupply,
            ...
            return
                _weightedMath.joinTokenInForExactBPTOut(
                    balances,
                    normalizedWeights,
                    totalSupply,
                    ManagedPoolStorageLib.getSwapFeePercentage(poolState),
            ...
            return _weightedMath.joinAllTokensInForExactBPTOut(balances, totalSupply, userData);
        } else {
    ...
    function _doExit(
        ...
        if (kind == WeightedPoolUserData.ExitKind.EXACT_BPT_IN_FOR_ONE_TOKEN_OUT) {
            return
                _weightedMath.exitExactBPTInForTokenOut(
                    balances,
                    normalizedWeights,
                    totalSupply,
                    ManagedPoolStorageLib.getSwapFeePercentage(poolState),
            ...
            return _weightedMath.exitExactBPTInForTokensOut(balances, totalSupply, userData);
        } else if (kind == WeightedPoolUserData.ExitKind.BPT_IN_FOR_EXACT_TOKENS_OUT) {
            return
                _weightedMath.exitBPTInForExactTokensOut(
                    balances,
                    normalizedWeights,
                    scalingFactors,
                    totalSupply,
