...
describe('NoProtocolFeesLiquidityBootstrappingPoolFactory', function () {
  ...
  sharedBeforeEach('deploy factory & tokens', async () => {
    ...
    factory = await deploy('NoProtocolFeeLiquidityBootstrappingPoolFactory', { args: [vault.address] });
  ...
  describe('disable', () => {
    it('begins enabled', async () => {
      expect(await factory.isDisabled()).to.equal(false);
    });
    ...
    context('when authorized', () => {
      ...
      it('emits an event', async () => {
        const receipt = await (await factory.connect(admin).disable()).wait();
        expectEvent.inReceipt(receipt, 'FactoryDisabled');
      });

      it('disabling prevents pool creation', async () => {
        await factory.connect(admin).disable();
        await expect(createPool()).to.be.revertedWith('DISABLED');
      });
    });
