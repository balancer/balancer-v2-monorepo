...
import LiquidityBootstrappingPool from '@balancer-labs/v2-helpers/src/models/pools/smart/LiquidityBootstrappingPool';
import { RawLiquidityBootstrappingPoolDeployment } from '@balancer-labs/v2-helpers/src/models/pools/smart/types';
...
describe('LiquidityBootstrappingPool', function () {
  ...
  context('for a 1 token pool', () => {
    it('reverts if there is a single token', async () => {
      ...
      await expect(LiquidityBootstrappingPool.create({ tokens, weights })).to.be.revertedWith('MIN_TOKENS');
    });
  });
  ...
  function itBehavesAsWeightedPool(numberOfTokens: number) {
    let pool: LiquidityBootstrappingPool, tokens: TokenList;
