...
describe('ControlledManagedPoolFactory', function () {
  let tokens: TokenList;
  let factory: Contract;
  let controlledFactory: Contract;
  ...
  sharedBeforeEach('deploy factory & tokens', async () => {
    ...
    controlledFactory = await deploy('ControlledManagedPoolFactory', { args: [factory.address] });
  ...
  describe('constructor arguments', () => {
    ...
    it('sets the base factory', async () => {
      expect(await controlledFactory.managedPoolFactory()).to.equal(factory.address);
    });

    it('registers the pool', async () => {
      expect(await controlledFactory.isPoolFromFactory(pool.address)).to.be.true;
      expect(await controlledFactory.isPoolFromFactory(assetManager.address)).to.be.false;
    });
