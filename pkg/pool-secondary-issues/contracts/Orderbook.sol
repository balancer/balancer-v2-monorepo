...
contract Orderbook is IOrder, ITrade, Ownable{
    ...
    function reportTrade(bytes32 _ref, bytes32 _cref, uint256 _price, uint256 securityTraded, uint256 currencyTraded) private {
        ...
        emit CallSwap(  _orders[_ref].swapKind==IVault.SwapKind.GIVEN_IN ? true : false,
                        _orders[_ref].tokenIn==_security ? "security" : "currency",
                        _orders[_ref].party, 
                        _orders[_cref].swapKind==IVault.SwapKind.GIVEN_IN ? true : false,
                        _orders[_cref].tokenIn==_security ? "security" : "currency",
                        _orders[_cref].party, 
                        oIndex
                    );
