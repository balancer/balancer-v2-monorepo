...
contract MockERC4626Token is TestToken, IERC4626 {
    ...
    function totalAssets() external view override returns (uint256){
        return ERC20(_depositAsset).balanceOf(address(this));
    }
    ...
    function deposit(uint256 assets, address receiver) external override returns (uint256) {
        ERC20(_depositAsset).transferFrom(msg.sender, address(this), assets);
    ...
    function redeem(uint256 shares, address receiver, address owner) external override returns (uint256) {
        ...
        // need update to work with totalSupply
        _burn(owner, shares);
        ERC20(_depositAsset).transfer(receiver, assets);
        return assets;
    }
