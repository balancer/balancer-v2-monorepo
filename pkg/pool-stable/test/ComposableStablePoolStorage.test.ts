...
import { bn, FP_SCALING_FACTOR } from '@balancer-labs/v2-helpers/src/numbers';
    ...
    describe('scaling factors', () => {
      describe('getScalingFactorX', () => {
        it('returns the correct scaling factor', async () => {
          const expectedScalingFactors = tokens.map((token) => FP_SCALING_FACTOR.mul(bn(10).pow(18 - token.decimals)));
          expectedScalingFactors.splice(bptIndex, 0, FP_SCALING_FACTOR);
